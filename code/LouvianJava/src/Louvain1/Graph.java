...
public class Graph {
    ...
    private HashMap<Integer, Double>[] weightsEdge;
    ...
    public Graph(int nNodes, double[] weightsNode, HashMap<Integer, Double>[] weightsEdge, ArrayList<Integer>[] adj, int nEdges) {
        this.nNodes = nNodes;
    ...
    public Graph(int nNodes, double[] weightsNode, HashMap<Integer, Double>[] weightsEdge,
    ...
    public boolean moveNode(double resolution, Random random) {
        ...
        do {
            ...
            for (int c : adj[j]) {
                ...
                weightsEdgeOfTempCluster[cluster[c]] += weightsEdge[c].get(j);
    ...
    public Graph getRebuildGraph() {
        ...
        reNet.weightsEdge = (HashMap<Integer, Double>[]) new HashMap[nClusters];
            ...
            for (int j = 0; j < nodesPerCluster[i].length; j++) {
                ...
                for (int n : adj[l]) {
                    if (cluster[n] != k) {
                        ...
                        if (reNet.weightsEdge[k] == null) {
                            reNet.weightsEdge[k] = new HashMap<>();
                        }
                        if (reNet.weightsEdge[k].get(cluster[n]) == null) {
                            temp = weightsEdge[l].get(n);
                        } else {
                            temp = reNet.weightsEdge[k].get(cluster[n]) + weightsEdge[l].get(n);
                        }
                        reNet.weightsEdge[k].put(cluster[n], temp);
    ...
    public static void main(String[] args) throws IOException{
        double resolution;
        Random random;
        String file = "D:\\Code\\py\\tf\\VGG16ConvolutionalAutoEncoder\\";
        random = new Random(100);
        String filename = "net";
        String filepath = file + filename + ".edgelist";
        String fileout = file + filename + "_out.txt";
        Graph testNet = ReadInput.readInputFile(filepath, " ", 0);
        ...
        System.out.printf("Edges Weights: %f\n", testNet.getTotalEdgeWeights());
        System.out.printf("NClusters: %d\n", testNet.getNClusters());
